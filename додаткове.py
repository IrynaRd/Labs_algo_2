...
class RedBlackQueue:
    ...
    def print_first_three(self):
        counter = [0]
        self.fix_print_first_three(self.root, counter)

    def fix_print_first_three(self, node, counter):
        if node != self.TNULL and counter[0] < 3:
            self.fix_print_first_three(node.left, counter)
            print(f"{node.data} - {node.priority}", end=",")
            counter[0] += 1
            self.fix_print_first_three(node.right, counter)

    def double_order(self, node):
        node.priority *= 2
        return node
    
    def insert_from_console(self):
        while True:
            input_data = input("Enter name and price of stock order via comma(in case you want to stop, enter 'end'): ")
            if input_data.lower() == "end":
                break
            data, priority = input_data.split(",")
...
q.print_queue()
print("Best stock orders:")
q.print_first_three()

print()
print("Best order:")
max_order = q.max_priority_node(q.root)
print(f"{max_order.data} - {max_order.priority}")

done = q.double_order(max_order)
print(f"executed order: {done.data} - {done.priority}")
q.remove(max_order)
print("\nNew queue: ")
q.print_queue()
